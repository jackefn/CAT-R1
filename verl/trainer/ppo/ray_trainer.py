...
def compute_data_metrics(batch, use_critic=True):
    ...
    answer_acc_scores = batch.batch['answer_acc_scores'].float()
    ...
    metrics = {
        ...
        **({
        ...
        # f1 score
        'critic/answer_acc_score/mean': torch.mean(answer_acc_scores).detach().item(),
        'critic/answer_acc_score/max': torch.max(answer_acc_scores).detach().item(),
        'critic/answer_acc_score/min': torch.min(answer_acc_scores).detach().item(),
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for batch_dict in self.val_dataloader:
            is_reset = True # 确保每个batch的初始化
            timing_raw = {}
            test_batch: DataProto = DataProto.from_single_dict(batch_dict)
            num_samples = batch_dict['input_ids'].shape[0]
            ...
            with _timer('step', timing_raw):
                ...
                with _timer('gen', timing_raw):
                    ...
                    final_gen_batch_output, select_final_questions, accs = generation_manager.run_llm_loop(
                ...
                # evaluate using reward_function
                try:
                    reward_tensor, answer_lst_acc , format_lst = self.val_reward_fn(test_batch, select_final_questions, accs)
                except:
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            data_source_acc[data_source].append(answer_lst_acc[i])
        ...
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/test_score/{data_source}'] = np.mean(rewards)
        for data_source, answers_acc in data_source_acc.items():
            metric_dict[f'val/answer_acc_score/{data_source}'] = np.mean(answers_acc)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs): # 默认为1
            count = 0 # 记录当前的训练步数
            for batch_dict in self.train_dataloader: # 加载当前batch的数据
                is_reset = True # 确保每个batch的初始化（初始化学生的选题记录）
                metrics = {}
                timing_raw = {}
                num_samples = batch_dict['input_ids'].shape[0]
                ...
                batch = batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n_repeat, interleave=True) # 每个问题重复5次，batch的长度为80
                ...
                gen_batch = batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids']) # 弹出所有数据的指定的键的内容

                with _timer('step', timing_raw=timing_raw):
                    first_input_ids = gen_batch.batch['input_ids'][:, -gen_config.max_start_length:].clone().long() # 获取初始输入
                    
                    # 5. Use expanded batch for generation
                    with _timer('gen', timing_raw):
                        generation_manager.timing_raw = timing_raw
                        final_gen_batch_output, select_final_questions,all_accs = generation_manager.run_llm_loop(
                            gen_batch=gen_batch,
                    ...
                    with torch.no_grad():
                        output = self.actor_rollout_wg.compute_log_prob(final_gen_batch_output) # 计算log_prob
                    ...
                    if self.use_reference_policy:
                        ...
                        with _timer('ref', timing_raw):
                            ref_log_prob = self.ref_policy_wg.compute_ref_log_prob(batch) # 计算参考策略的log_prob
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, answer_lst_acc, format_lst = self.reward_fn(batch,select_final_questions,all_accs)
                        batch.batch['token_level_scores'] = reward_tensor
                        batch.batch['answer_acc_scores'] = torch.tensor(answer_lst_acc)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        # update actor
                        with _timer('update_actor', timing_raw): # 更新策略模型
