...
from cat.CAT.model.abstract_model import AbstractModel
from cat.CAT.dataset import AdapTestDataset, TrainDataset, Dataset
...
class IRT(nn.Module):
    def __init__(self, num_students, num_questions, num_dim):
        ...
        self.theta = nn.Embedding(self.num_students, self.num_dim) # 学生潜在特征   
        self.alpha = nn.Embedding(self.num_questions, self.num_dim) # 问题区分度
        self.beta = nn.Embedding(self.num_questions, 1) # 问题难度

        for name, param in self.named_parameters():
            if 'weight' in name:
                nn.init.xavier_normal_(param) # 正态分布初始化
...
class IRTModel(AbstractModel):
    ...
    def init_model(self, data: Dataset):
        ...
        self.policy = StraightThrough(data.num_questions, data.num_questions,policy_lr, self.config) # 加载策略
        self.n_q = data.num_questions

    def train(self, train_data: TrainDataset, k_folds=5, log_step=1):
        ...
        fold_sizes = torch.full((k_folds,), dataset_size // k_folds, dtype=torch.int32)
        ...
        for fold in range(k_folds):
            ...
            # 划分训练集和验证集
            valid_indices = indices[start_idx:end_idx]
            train_indices = torch.cat([indices[:start_idx], indices[end_idx:]])
            
            train_subset = data.Subset(train_data, train_indices)
            valid_subset = data.Subset(train_data, valid_indices)
            ...
            for ep in range(1, epochs + 1):
                ...
                for cnt, (student_ids, question_ids, _, labels) in enumerate(train_loader):
                    ...
                    train_loss += bz_loss.item()
                    if cnt % log_step == 0:
                        logging.info(f'Fold [{fold+1}/{k_folds}] Epoch [{ep}/{epochs}] Batch [{cnt}]: loss={train_loss/(cnt+1):.5f}')
                
                # 验证阶段
                valid_metrics = self._validate(valid_loader, device)
                logging.info(f'Fold [{fold+1}/{k_folds}] Epoch [{ep}/{epochs}] Validation: loss={valid_metrics["loss"]:.5f}, auc={valid_metrics["auc"]:.5f}')
        ...
        if best_model_state is not None:
            ...
            logging.info(f'最终使用最佳验证集AUC: {best_valid_auc:.5f}的模型')

    def _validate(self, valid_loader, device):
        """验证函数"""
        self.model.eval()
        total_loss = 0
        all_preds = []
        ...
        with torch.no_grad():
            for student_ids, question_ids, _, labels in valid_loader:
                student_ids = student_ids.to(device)
                question_ids = question_ids.to(device)
                labels = labels.to(device).float()
    ...
    def _loss_function(self, pred, real):
        return -(real * torch.log(0.0001 + pred) + (1 - real) * torch.log(1.0001 - pred)).mean() # 二元交叉熵损失
