...
from cat.CAT.dataset import AdapTestDataset,Dataset
from cat.CAT.model.IRT import IRT,IRTModel
import os
...
class NCAT(nn.Module):
    def __init__(self, n_question, d_model=10, n_blocks=1,kq_same=True, dropout=0.0, policy_fc_dim=512, n_heads=1, d_ff=2048,  l2=1e-5, separate_qa=None, pad=0): # NCAT模型骨干
        super().__init__()
        """
        Input:
...
class env:
    def __init__(self,data,concept_map,config,T):
        ...
        name = 'IRT'
        self.model, self.dataset = self.load_CDM(name,data,pth_path,config) # 加载CDM
        #print(self.model)
...
class NCATModel():
    ...
    def __init__(self, NCATdata,concept_map,config,test_length):
        ...
        self.config = config
        self.model = None
        self.env = env(data=NCATdata,concept_map=concept_map,config=config,T=test_length) # 环境为学生测试数据
        self.memory = []
        self.item_num =self.env.item_num
        self.user_num = self.env.user_num
        self.device = config['device']
        self.fa = NCAT(n_question=NCATdata.num_questions+1).to(self.device) # 初始化NCAT模型
    ...
    def ncat_policy(self,sid,THRESHOLD,used_actions,type,epoch):
        ...
        state = self.env.reset_with_users(sid) # 重置环境
        while not done:
            data = {"uid": [state[0][1]]}
            for i in range(2):
            ...
            policy = self.fa.predict(data)[0] # 预测下一个选择的题目
            if type == "training":
